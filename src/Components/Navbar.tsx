...
export default function Navbar() {
  ...
  const handleSwipe = () => {
    ...
    if (!isOpen && touchStartX.current < 30 && distance > 80) {
      setIsOpen(true); // open on right swipe from edge
    } else if (isOpen && distance < -80) {
      setIsOpen(false); // close on left swipe
    }
  };
  useEffect(() => {
    ...
    <div>
      {
        <div
          ...
          className={` z-3 block border-r border-r-[#b9b5b5ef] left-0 top-[55px] bottom-0  py-4 px-6  fixed xl:w-[260px] w-[150px] ${windowWidth < 768 && (isOpen ? "translate-x-0" : "-translate-x-full")}
         `}>
          <div
            ...
            <div className="w-full border-t-[#b9b5b5ef] border-t-1 mt-5 flex flex-col items-start  gap-6 pt-4">
              ...
              <div ref={el => appearanceRef.current['second'] = el} className="flex gap-2 items-center relative">
                <Appearance />
                <span onClick={() => { setShowMode(!showMode) }}>Change Appearance</span>
